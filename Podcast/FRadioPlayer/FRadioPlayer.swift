...
open class FRadioPlayer: NSObject {
    ...
    /// AVPlayer
    private var player: AVPlayerV2?
    
    /// Last player item
    private var lastPlayerItem: AVPlayerItemV2?
    ...
    private func setupPlayer(with asset: AVURLAsset) {
        ...
        playerItem = AVPlayerItemV2(asset: asset, automaticallyLoadedAssetKeys: requiredAssetKeys)
    }
    ...
    private func playerItemDidChange() {
        ...
        if let item = lastPlayerItem {
            ...
            item.removeObserver(self, forKeyPath: #keyPath(AVPlayerItemV2.status))
            item.removeObserver(self, forKeyPath: #keyPath(AVPlayerItemV2.isPlaybackBufferEmpty))
            item.removeObserver(self, forKeyPath: #keyPath(AVPlayerItemV2.isPlaybackLikelyToKeepUp))
            item.removeObserver(self, forKeyPath: #keyPath(AVPlayerItemV2.duration))
        ...
        if let item = playerItem {
            ...
            item.addObserver(self, forKeyPath: #keyPath(AVPlayerItemV2.status), options: [.old, .new], context: &playerItemContext)
            item.addObserver(self, forKeyPath: #keyPath(AVPlayerItemV2.isPlaybackBufferEmpty), options: [.old, .new], context: &playerItemContext)
            item.addObserver(self, forKeyPath: #keyPath(AVPlayerItemV2.isPlaybackLikelyToKeepUp), options: [.old, .new], context: &playerItemContext)
            item.addObserver(self, forKeyPath: #keyPath(AVPlayerItemV2.duration), options: [.old, .new], context: &playerItemContext)
    ...
    override open func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if let item = object as? AVPlayerItemV2, let keyPath = keyPath, item == self.playerItem {
            
            switch keyPath {
                
            case #keyPath(AVPlayerItemV2.status):
            ...
            case #keyPath(AVPlayerItemV2.isPlaybackBufferEmpty):
            ...
            case #keyPath(AVPlayerItemV2.isPlaybackLikelyToKeepUp):
...
extension FRadioPlayer: AVPlayerV2Delegate {
    
    func tuneAvailable(_ player: AVPlayerV2, matches: [TuneURL.Match]) {
        notifiyObservers { observer in
            observer.radioPlayer(self, tuneAvailable: matches)
        }
    }
}
